...
<div class="container">
			...
			<div class="filter-group">
				...
				<select id="filter-autor" name="autor">
					...
					foreach ($autores as $autor) {
						...
						echo '<option value="' . $autor->ID . '" ' . $selected . '>' . esc_html($autor->display_name) . '</option>';
					}
					?>
				</select>
			</div>
	...
	if ($autor_get) {
		$meta_query[] = array(
			'key' => '_artigo_autores',
			'value' => serialize(strval($autor_get)),
			'compare' => 'LIKE'
		);
	}
						...
						?>
							<div class="artigo-autores">
								<?php foreach ($autores_ids as $autor_id) :
									...
									if ($autor_info) :
										$autor_nome = $autor_info->display_name;
										$autor_avatar_url = get_avatar_url($autor_id, array('size' => 96));
										$autor_link = get_author_posts_url($autor_id);
								?>
										<div class="autor-avatar">
											<a href="<?php echo esc_url($autor_link); ?>">
